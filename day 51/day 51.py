...
folder_path = r"C:\Users\LEOVO\Documents\GitHub\100_days_of-_python\day 51"
...
while True:
    ...
    if menu == "add":
        ...
        if item in mytodo:
            ...
            mytodo[index] = input("What do you want to replace it with? > ")
            due[index] = input("When is it due? > ")
            importance = input(f"\033[0;36m How important is it (high, medium, low)? >\033[0m \n").lower()
            for level in priority:
                if item in priority[level]:
                    priority[level].remove(item)
            priority[importance].append(mytodo[index])
        else:
